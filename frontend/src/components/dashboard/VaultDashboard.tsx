...
export const VaultDashboard: React.FC<VaultDashboardProps> = ({
  ...
  const priorityIssues = healthSummary?.issues.slice(0, 4) || []
  const quickEntries = filteredEntries.slice(0, 6)
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1.6fr)_360px]">
        <div className="space-y-6">
            ...
            <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
              <div className="max-w-2xl">
                ...
                <p className="mt-3 max-w-xl text-sm leading-6 text-slate-600 dark:text-slate-300">
                  Your encrypted workspace is live. Monitor password posture, identity readiness, device trust, and recent vault activity from one surface.
                </p>
              </div>
            ...
            <div className="mt-6 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
              <div className={metricCardClass}>
                <div className="flex items-center justify-between">
                  <span className="text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-400">Entries in Vault</span>
                  <Layers3 className="h-4 w-4 text-slate-400" />
                </div>
                <p className="mt-4 text-4xl font-semibold tracking-[-0.04em] text-slate-950 dark:text-white">{entries.length}</p>
                <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">{filteredEntries.length} visible in current view</p>
              </div>
              ...
              <div className={metricCardClass}>
                ...
                <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">{allTags.length} tags and {topCategories.length} active categories mapped</p>
              </div>
            </div>
          </motion.section>
          ...
          <section className="grid gap-6 xl:grid-cols-[minmax(0,1.1fr)_minmax(280px,0.9fr)]">
            <div className={sectionCardClass}>
              <div className="flex items-start justify-between gap-4">
                <div>
                  ...
                  <h3 className="mt-2 text-xl font-semibold text-slate-950 dark:text-white">Risk & response matrix</h3>
                  <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">Triage exposed credentials, launch scans, and route remediation actions quickly.</p>
                </div>
              ...
              <div className="mt-5 grid gap-3 sm:grid-cols-3">
                {[
                  ...
                  <div key={stat.label} className="rounded-[24px] border border-slate-200 bg-slate-50/80 p-4 dark:border-slate-800 dark:bg-slate-900/70">
              ...
              <div className="mt-5 space-y-3">
                {priorityIssues.length === 0 ? (
                  <div className="rounded-[24px] border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-800 dark:border-emerald-900 dark:bg-emerald-950/30 dark:text-emerald-200">
                    No high-priority password risks are open right now.
                  </div>
                ) : (
                  priorityIssues.map((issue) => (
                    <div key={`${issue.entryId}-${issue.type}`} className="rounded-[24px] border border-slate-200 p-4 dark:border-slate-800">
              ...
              <div className="mt-5 flex flex-wrap gap-3">
                <Button onClick={onRunBreachScan} variant="primary" size="sm" loading={isScanningBreaches}>
                  <ScanSearch className="h-4 w-4" />
                  {isScanningBreaches ? 'Scanning breach corpus' : 'Run breach scan'}
                </Button>
                <Button onClick={onStrengthenPasswords} variant="outline" size="sm">
                  <Sparkles className="h-4 w-4" />
                  Strengthen weak passwords
                </Button>
                <Button onClick={onOpenPasswordGenerator} variant="outline" size="sm">
            ...
            <div className={sectionCardClass}>
              <div className="flex items-start justify-between gap-4">
                <div>
                  <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-400">Coverage map</p>
                  <h3 className="mt-2 text-xl font-semibold text-slate-950 dark:text-white">Category density</h3>
                  <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">See where your vault is concentrated and which tags dominate the workspace.</p>
                </div>
              </div>
          ...
          <section className={sectionCardClass}>
            <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
              <div>
                <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-400">Vault inventory</p>
                <h3 className="mt-2 text-xl font-semibold text-slate-950 dark:text-white">Recent credentials & secure notes</h3>
                <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">Your most relevant entries, staged for quick triage and edit operations.</p>
              </div>
                ...
                {quickEntries.map((entry) => {
                  ...
                  return (
                    <motion.div
                      key={entry.id}
                      layout
                      className="rounded-[26px] border border-slate-200 bg-[#fafbf8] p-5 transition-colors hover:border-emerald-300 dark:border-slate-800 dark:bg-slate-900/70 dark:hover:border-emerald-900"
                    >
                      <div className="flex items-start justify-between gap-3">
        ...
        <div className="space-y-6">
          ...
          <section className={sectionCardClass}>
            <div className="flex items-start justify-between gap-3">
              <div>
                ...
                <h3 className="mt-2 text-xl font-semibold text-slate-950 dark:text-white">Identity & operations</h3>
              </div>
            </div>
            <div className="mt-5 space-y-3">
              {[
                { label: 'Manage passkeys', description: 'Enroll or rotate WebAuthn credentials.', action: onOpenPasskeys, icon: <KeyRound className="h-4 w-4" /> },
          ...
          <section className={sectionCardClass}>
            <div className="flex items-start justify-between gap-3">
              <div>
                <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-400">Readiness queue</p>
                <h3 className="mt-2 text-xl font-semibold text-slate-950 dark:text-white">What needs attention</h3>
              </div>
            </div>
            <div className="mt-5 space-y-3">
              {[
                ...
                },
                {
                  ...
                  detail: weakEntries > 0 ? `${weakEntries} entries fall below the target length/complexity threshold.` : 'Most stored passwords meet the current strength heuristic.',
                ...
                },
                {
                  ...
                  detail: totpEntries > 0 ? `${totpEntries} entries already include TOTP material.` : 'Add more MFA-backed entries or passkeys for stronger account recovery.',
