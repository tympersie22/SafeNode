    ...
    <AnimatePresence>
      {isOpen && (
        <>
          ...
          >
            <div className="glass rounded-2xl border border-white/20 max-w-2xl w-full max-h-[90vh] overflow-hidden shadow-2xl shadow-black/10" onClick={(e) => e.stopPropagation()}>
        {/* Header */}
        <div className="px-6 py-4 border-b border-white/20 bg-gradient-to-r from-white/50 to-white/30">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <motion.button
                onClick={onClose}
                className="p-2 rounded-xl text-slate-500 hover:bg-white/50 hover:text-slate-700 transition-all duration-200"
              ...
              <h2 id="entry-form-title" className="text-xl font-bold bg-gradient-to-r from-slate-900 to-slate-700 bg-clip-text text-transparent">
                {entry ? 'Edit Entry' : 'Add New Entry'}
              </h2>
            </div>
            <motion.button
              onClick={onClose}
              className="p-2 hover:bg-white/50 rounded-xl transition-all duration-200 text-slate-500 hover:text-slate-700"
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
        ...
        <form onSubmit={handleSubmit} className="p-4 space-y-4 overflow-y-auto max-h-[calc(90vh-120px)]">
          ...
          <div>
            ...
            <input
              ...
              className="w-full px-2.5 py-1.5 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-purple-500 transition font-mono text-xs"
          ...
          <div>
            ...
            <div className="flex space-x-2">
              <input
                ...
                className="flex-1 px-2.5 py-1.5 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-purple-500 transition"
                placeholder="Add tag..."
              />
              <motion.button
                type="button"
            ...
            {attachments.length > 0 && (
              <div className="mt-3 border border-slate-200 rounded-lg overflow-hidden">
                {attachments.map((attachment) => (
                  <div
                    key={attachment.id}
                    className="flex items-center justify-between gap-3 px-3 py-2 text-xs bg-white hover:bg-slate-50 border-b border-slate-200 last:border-b-0"
                  >
                    <div>
        ...
        {/* Footer */}
        <div className="px-4 py-3 border-t border-slate-200 bg-gradient-to-r from-white to-slate-50 flex justify-end space-x-2">
          <motion.button
            type="button"
            onClick={onClose}
