...
exports.registerResendWebhookRoutes = registerResendWebhookRoutes;
const webhookIdempotencyService_1 = require("../services/webhookIdempotencyService");
const resendWebhookService_1 = require("../services/resendWebhookService");
async function registerResendWebhookRoutes(server) {
    server.post('/api/webhooks/resend', { config: { rawBody: true } }, async (request, reply) => {
        ...
        const signatureValid = (0, resendWebhookService_1.verifyResendWebhookSignature)(rawBody, { id, timestamp, signature }, webhookSecret);
        if (!signatureValid) {
            return reply.code(400).send({
        ...
        const status = await (0, webhookIdempotencyService_1.recordWebhookEvent)('resend', eventId, rawBody);
        if (status === 'duplicate') {
