...
async function createPasswordResetToken(email) {
    ...
    // Find user by email
    const user = await (0, userService_1.findUserByEmail)(email.toLowerCase().trim());
    if (!user) {
    ...
    // Send reset email
    try {
        await emailService_1.emailService.sendPasswordResetEmail(user.email, token, user.displayName);
    }
    catch (error) {
