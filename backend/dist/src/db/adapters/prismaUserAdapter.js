 ...
 */
function prismaUserToDomain(prismaUser) {
    const user = {
        id: prismaUser.id,
        ...
        // Device limits
        devices: (prismaUser.devices || []).map((d) => ({
            id: d.deviceId,
            name: d.name,
    ...
    user.tokenVersion = prismaUser.tokenVersion || 1;
    return user;
}
/**
 * Prisma-based User Database Adapter
 */
exports.prismaUserAdapter = {
    /**
     * Create a new user
     */
    async create(user) {
        ...
        try {
            ...
            result.tokenVersion = verified.tokenVersion || 1;
            return result;
        }
        catch (error) {
            console.error('[PrismaAdapter] Error creating user:', {
                ...
                email: normalizedEmail,
                userId: user.id,
                prismaError: error instanceof client_1.Prisma.PrismaClientKnownRequestError ? {
                    code: error.code,
                    meta: error.meta
                } : null
            });
    ...
    async findById(id) {
        ...
        result.tokenVersion = user.tokenVersion || 1;
        return result;
    },
    /**
     * Find user by email
    ...
    async update(id, input) {
        ...
        if (input.lastLoginAt !== undefined)
            updateData.lastLoginAt = input.lastLoginAt ? new Date(input.lastLoginAt) : null;
        if (input.vaultSalt !== undefined)
            updateData.vaultSalt = input.vaultSalt;
        if (input.vaultEncrypted !== undefined)
            updateData.vaultEncrypted = input.vaultEncrypted;
        if (input.vaultIV !== undefined)
            updateData.vaultIV = input.vaultIV;
        if (input.vaultVersion !== undefined)
            updateData.vaultVersion = input.vaultVersion;
        ...
        if (input.biometricEnabled !== undefined)
            updateData.biometricEnabled = input.biometricEnabled;
        if (input.subscriptionTier !== undefined)
            updateData.subscriptionTier = input.subscriptionTier;
        if (input.subscriptionStatus !== undefined)
            updateData.subscriptionStatus = input.subscriptionStatus;
        ...
        if (input.tokenVersion !== undefined)
            updateData.tokenVersion = input.tokenVersion;
        ...
        result.tokenVersion = updated.tokenVersion || 1;
        return result;
    },
    /**
     * Delete user
